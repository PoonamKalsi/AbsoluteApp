...
{
    public class InsertNewLockController : ApiController
    {
        // GET api/<controller>
        public HttpResponseMessage Get(HttpRequestMessage request, string user, string BatchId)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("Insert into LockPicklist (BatchId,UserName,CreatedOn) values('" + BatchId+"','"+user+"',GetDate())", con))
                {
                ...
                sucessresponse.message = "Successfully inserted";
