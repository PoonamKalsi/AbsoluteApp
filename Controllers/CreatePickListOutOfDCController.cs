...
{
    public class CreatePickListOutOfDCController : ApiController
    {
        public HttpResponseMessage Get(HttpRequestMessage request, string BatchId, string DCs, string UserId = "")
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SplitPicklistAsPerDC", con1))
                    {
                        if (con1.State == System.Data.ConnectionState.Closed)
                            con1.Open();
                        cmd.Parameters.AddWithValue("@BatchId", BatchId);
                        cmd.Parameters.AddWithValue("@locations", DCs);
