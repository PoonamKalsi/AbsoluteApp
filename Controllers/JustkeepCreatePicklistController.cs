...
{
    public class JustkeepCreatePicklistController : ApiController
    {
        // GET api/<controller>
        public HttpResponseMessage Get(HttpRequestMessage request, string type, string UserId = null)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("select * from JustkeepPickList where Status = 'In Progress'", con))
                {
                ...
                if (dt.Rows.Count == 0)
                {
                    using (SqlCommand cmd = new SqlCommand("Insert into JustkeepPickList (BatchId,[Status],[Request Type],CreatedOn,CreatedByUser) values('" + i + "', 'Pending', '" + type + "', GetDate(),'" + UserId + "')", con))
                    {
                        if (con.State == ConnectionState.Closed)
                            con.Open();
                    ...
                    using (SqlCommand cmd = new SqlCommand("update JustkeepPickList set Status='Processed',ProcessedDate='" + DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss tt") + "' where BatchID='" + i + "'"))
                    {
                        if (con.State == ConnectionState.Closed)
                            con.Open();
                        cmd.Connection = con;
                ...
                Process p1 = new Process();
                p1.StartInfo.FileName = @"H:\Applications\JustKeeper\JUSTKEEPER'S APP WORK\PICKLIST ORDER STATUS UPDATE\Justkeep_Order_Status_Update.exe";
                p1.StartInfo.Arguments = i;
                p1.Start();
