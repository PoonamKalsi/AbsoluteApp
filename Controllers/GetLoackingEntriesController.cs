...
{
    public class GetLoackingEntriesController : ApiController
    {
        // GET api/<controller>
        public HttpResponseMessage Get(HttpRequestMessage request)
        {
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand("select * from LockPicklist", con))
                {
                ...
                IList<entries> ent = new List<entries>();
                foreach (DataRow r in dt.Rows)
                {
                    entries en = new entries();
                    en.id = r["Id"].ToString();
                    en.BatchId = r["BatchId"].ToString();
                    en.UserName = r["UserName"].ToString();
                    en.CreatedOn = r["CreatedOn"].ToString();
                    ent.Add(en);
                }
                SplitPickListsucessresponse sucessresponse = new SplitPickListsucessresponse();
                sucessresponse.message = ent;
        ...
        {
            public IList<entries> message { get; set; }
        }
